...
vec3 ComputeSkyColor(vec3 rayDirection) {
    ...
    const float I_sun = 20;                                   // Intensity of the sun
    const vec3 beta_R = vec3(5.8e-5f, 13.5e-5f, 33.1e-5f);    // Rayleigh scattering coefficient
    const float beta_M = 21e-5f;                              // Mie scattering coefficient
    const float H_R = (uInnerHeight + uOuterHeight) * 0.1332; // Rayleigh scale height
    const float H_M = (uInnerHeight + uOuterHeight) * 0.0200; // Mie scale height
    const float g = 0.888;                                    // Mie scattering direction - 
                                                              //  - anisotropy of the medium

    // Normalize the light direction
    vec3 sunDir = normalize(-GetSunLight().dir);
    ...
    hit = RaySphereIntersection(
        uCameraPosition, rayDirection,
        uSphereCenter, uSphereRadius,
        t1, position
    );
    float t = hit ? min(t0, t1) : t0;

    float segmentLen = t / float(viewSamples);
    ...
    for (int i = 0; i < viewSamples; ++i)
    {
        ...
        vec3 vSample = uCameraPosition + rayDirection * (tCurrent + segmentLen * 0.5);

        // Height of the sample above the planet
        float height = length(vSample - uSphereCenter) - uSphereRadius;
        ...
        float segmentLenLight = t / float(lightSamples);
        ...
        for (int j = 0; j < lightSamples; ++j)
        {
            ...
            float heightLight = length(lSample - uSphereCenter) - uSphereRadius;
